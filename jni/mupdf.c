...
#define JNI_FN(A) Java_com_csu_powerpdf_ ## A
#define PACKAGENAME "com/csu/powerpdf"
...
//insert audio
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_insertAudio)(JNIEnv *env,jobject thiz,jstring sound_path,jstring icon_path,jint pageNum,jint x,jint y,jint pageWidth,jint pageHeight)
{
	globals *glo = get_globals(env,thiz);
	char * str_sound;
	char * str_icon;
//	str_sound = jstringTostring(env,sound_path);
//	str_icon = jstringTostring(env,icon_path);
	str_sound = (*env)->GetStringUTFChars(env, sound_path, NULL);
	str_icon = (*env)->GetStringUTFChars(env,icon_path,NULL);
	pdf_document *idoc = pdf_specifics(glo->doc);
	pdf_page *page = pdf_load_page(idoc,pageNum);
	fz_rect page_rect = page->mediabox;
	float page_width = page_rect.x1-page_rect.x0;
	float page_height = page_rect.y1-page_rect.y0;
	float scale_width = pageWidth/page_width;
	float scale_height = pageHeight/page_height;
	fz_rect icon_rect = {x/scale_width-16,y/scale_height-20,x/scale_width+16,y/scale_height+12};
	LOGE("pageHeight-->%d",pageHeight);
	LOGE("page_height-->%f",page_height);
	LOGE("x/scale_width-->%f",x/scale_width);
	LOGE("y/scale_height--->%f",y/scale_height);

	pdf_set_multimedia_ap(str_icon);
	pdf_add_multimedia(idoc,pageNum,icon_rect,str_sound);
	(*env)->ReleaseStringUTFChars(env, sound_path, str_sound);
	(*env)->ReleaseStringUTFChars(env, icon_path, str_icon);
}

JNIEXPORT jbyteArray JNICALL
JNI_FN(MuPDFCore_playAudio)(JNIEnv *env, jobject thiz,jint pageNum,jint x,jint y)
{
	globals *glo = get_globals(env,thiz);
	pdf_document *idoc = pdf_specifics(glo->doc);
	pdf_page *page = pdf_load_page(idoc,pageNum);
...
//	LOGE("play->point.x: %d point.y: %d",point.x,point.y);
	pdf_annot * annot = NULL;
	annot = pdf_get_multimedia_annot(idoc,pageNum,point);
	int length;
	jbyteArray bytes = NULL;
	if(annot!=NULL)
	{
		unsigned char * data = pdf_get_multimedia_data(idoc,annot,&length,NULL);
		//建立byte数组
		bytes = (*env)->NewByteArray(env,length);
		//将char* 转换为byte数组
		(*env)->SetByteArrayRegion(env,bytes, 0, length, (jbyte*)data);
		free(data);
	}
	return bytes;
}
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addWatermarkWithText)(JNIEnv * env,jobject thiz, jstring string)
{
//	globals *glo = get_globals(env, thiz);
//	char * str;
//	str = (*env)->GetStringUTFChars(env, string, NULL);
//	pdf_document *idoc = pdf_specifics(glo->doc);
//	fz_rect rect = {100,100,300,300};
//	pdf_create_watermark_with_string(idoc,str,rect);
//	(*env)->ReleaseStringUTFChars(env, string, str);
}

//add watermark with image
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addWatermarkWithImage)(JNIEnv * env,jobject thiz,jint pageNum, jbyteArray data, jint width, jint height, jint pageWidth, jint pageHeight,jint length)
{
	globals *glo = get_globals(env, thiz);
	jbyte* arr = (*env)-> GetByteArrayElements(env,data,NULL);
	unsigned char* image_data = (unsigned char *)arr;
	pdf_document *idoc = pdf_specifics(glo->doc);
	fz_rect rect = {0,0,width,height};
	pdf_create_watermark_with_img(idoc,pageNum,image_data,width,height,&rect,length);
}

//判断是否有水印
JNIEXPORT jint JNICALL
JNI_FN(MuPDFCore_isPageHasWatermark)(JNIEnv * env, jobject thiz, jint pageNum)
{
	globals *glo = get_globals(env, thiz);
	pdf_document *idoc = pdf_specifics(glo->doc);
	pdf_page_has_watermark(idoc,pageNum);
}
