...
public class MuPDFPageView extends PageView implements MuPDFView {
	...
	//define the selected annotataion
	...
	public void setSelectedAnnotation(Annotation mSelectedAnnotation){
		this.mSelectedAnnotation = mSelectedAnnotation;
	}
	...
	public Hit passClickEvent(float x, float y) {
		...
		if (mAnnotations != null) {
			float min=10000f;
			float delta = 0.0f;
			int selectIndex = 0;//标记点击的annotation
			for (i = 0; i < mAnnotations.length; i++)
				if (mAnnotations[i].contains(docRelX, docRelY)) {
//					Log.d("weiquanyun","MuPDFPageView.passClickEvent-->x = "+docRelX+" y = "+docRelY+" "+mAnnotations[i].left);
					mAnnotations[i].setPoint(docRelX, docRelY);
					hit = true;
					delta = Math.abs(docRelX-mAnnotations[i].left);
//					Log.d("weiquanyun","i = "+i+" delta = "+delta+" min = "+min);
					if(min>delta)
					{
						min = delta;
						selectIndex = i;
...
//					break;
			...
			if (hit) {
				switch (mAnnotations[selectIndex].type) {
				case TEXT:
				case HIGHLIGHT:
				case UNDERLINE:
				case SQUIGGLY:
				...
				case INK:
				case SCREEN:
					mSelectedAnnotationIndex = selectIndex;
					setItemSelectBox(mAnnotations[selectIndex]);
					setSelectedAnnotation(mAnnotations[selectIndex]);
